...
取槽道半高 \(h=1\)、体积平均速度 \(U_b\)、体积平均密度
\(\rho_b\) 作参考量：

\[
L_{ref}=h=1,\quad U_{ref}=U_b=1,\quad \rho_{ref}=\rho_b=1,
\quad T_{ref}=T_w=1.
\]
...
\[
Re_b^{(h)}=\frac{\rho_b U_b h}{\mu_b}=U_b^+Re_\tau.
\]

本例初始速度型积分给出 \(U_b^+=15.481978793165828\)，因而

\[
Re_b^{(h)}=2786.756182769849,qquad
\mu_{ref}=1/Re_b^{(h)}=3.588401476178181\times10^{-4}.
\]
...
本例设体系马赫数 \(Ma_b=0.2\)。程序从参考量导出 Ma，因此配置使用

\[
R=17.857142857142858,quad \gamma=1.4,quad T_{ref}=1,quad U_{ref}=1,
\]

使 \(c_{ref}=\sqrt{\gamma R T_{ref}}=5\)、\(Ma_b=U_{ref}/c_{ref}=0.2\)。压力以
\(\rho_{ref}U_{ref}^2\) 无量纲化，所以初始 \(\rho=T=1\) 对应
\(p=1/(\gamma Ma_b^2)=17.8571428571\)。这是低但非零 Ma 的可压缩槽道；当前配置
没有打开低 Mach 预处理。
...
\[
a_x=\left(\frac{u_\tau}{U_b}\right)^2
...
所监测的原 zone 零基 cell-j 层是：

| j | 单元中心 y | 设计 \(y_w^+\) |
|---:|---:|---:|
| 0, 47 | -0.995438, +0.995438 | 0.821 |
| 2, 45 | -0.973746, +0.973746 | 4.726 |
| 5, 42 | -0.928224, +0.928224 | 12.920 |
| 11, 36 | -0.766784, +0.766784 | 41.979 |
| 17, 30 | -0.468880, +0.468880 | 95.602 |
| 23, 24 | -0.038712, +0.038712 | 173.032 |
...
体平均速度为 1.0003315，与连续目标 1 的差别是稀疏网格离散积分误差。
...
`output.statistics.xz_planes.cell_j_indices` 中的 j 是每个原生 CGNS zone 的零基单元索引，
不是顶点索引或 MPI 切分后的局部索引。每个 j 自动追加

\[
\langle u\rangle_{xz,j}=\frac{\sum A_{ik}u_{ijk}}{\sum A_{ik}},\qquad
Q_{xz,j}^{(x)}=\sum A_{ik}\rho_{ijk}u_{ijk}.
\]

`xz_mean_u_j*` 是该 y 位置的瞬时 x-z 平面平均流向速度。
`xz_mass_flow_x_j*` 是壁平行 x-z 面上 \(\rho u\) 的面积积分指标，**不是穿过该面的法向流量**。
对于周期充分发展槽道，它可用来跟踪各 y 层的平均速度/质量速度型；若需要
穿过固定 y-z 截面的总质量流量，当前内建接口还没有提供。
...
| 终止步数/时间 | 5 / 0.001175229194474591 |
| 每步 dt | 约 0.000235046 |
| 重构/Riemann 回退 | 0 / 0（所有五步） |
| 终场最小 \(\rho\) | 0.9995082018603285 |
| 终场最小 \(p\) | 17.84484915370814 |
| 终场最小 \(T\) | 0.9998032539880043 |
| 初始/终止实测 \(Re_\tau\) | 177.9758103 / 178.1018709 |
| 初始/终止平均 \(|\tau_w|\) | 0.0040787211 / 0.0040844961 |
| 初始/终止总质量 | 39.4784176043553 / 39.4784176043563 |

初始离散实测 \(Re_\tau\) 比设计值低约 1.12%，来自稀疏壁法向网格上的单边导数离散。
下/上壁初值分别为 0.004078721114696877 和 0.004078721114696770，对称性达到浮点精度。

这些数据支持如下结论：CGNS 周期多块网格、三维槽道初场、两面等温壁、常体积力、
粘性 WCNS、4-rank 通信、多层平面统计、壁摩擦统计、CGNS 场/检查点输出在短时推进中可用。
...
为使配置真正可执行，本阶段对程序做了三类显式修改，不存在隐藏修改：
...
体积力、x-z 多层统计、周期连接、粘性通量、MPI 分区、输出与重启均是已有通用功能，
本 case 只对它们进行组合和验证。
...
- 是否清楚区分 x-z 壁平行积分指标与真正 y-z 截面流量；
- 是否接受“5 步工程可行性通过，湍流/DNS 物理验收未开始”的结论；
- 长算前是否需要先新增真正 y-z 截面流量、累积时间平均/RMS/Reynolds 应力或恒流量控制器。
