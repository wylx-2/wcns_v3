...
- 新增 `docs/development-roadmap.md`，从阶段 H 起强制执行。
- 固定阶段 H--M 的范围、小任务、专项数值验收、串行/MPI 回归、Git/GitHub 分支与标签规则。
- 正式验收无论成功或失败都必须提交验收报告；失败时不创建候选标签、不合并 `main`、不进入下一阶段。
- 自动验收通过只产生候选版本；必须由项目负责人明确批准后才能合并并开始下一阶段。
