...
阶段 H 的正式自动验收通过，候选版本可提交项目负责人进行人工检查。阶段 H 没有实现阶段 I 的高阶度量，也没有实现阶段 J 的无粘 WCNS/实际源项求值。人工卡口 G8 尚未通过，因此不得合并 `main`，也不得进入阶段 I。
...
请项目负责人检查 `stage-h-candidate-v1`。在收到明确批准前，分支保持独立，不合并 `main`，不创建阶段 I 分支。
