...
状态：2026-09-11 按 P/Q 连续推进授权冻结并实施；Q 候选完成后必须停在进入 R 前的人工卡口。

## 1. 目标、权威公式与非目标

本阶段实现《算法补充》11.1--11.3，尤其以 11.2.3 的 troubled cell 与离散支持传播为权威
算法。目标是在每个 SSPRK3 候选提交前验证物理容许性，只升级能影响失败单元的无粘面通量；
...
1. 由权威残差 stencil 得到每个 troubled cell 的全部面支持；
2. 本地面请求前进一个有效级别；
3. 所有 rank 反向交换请求并在 owner 真实面取最大值；
4. 若全局没有 owner 面变化，则本阶段不能再局部修复，拒绝整步；
5. 以未改变的阶段输入重新计算状态/边界/完整面通量/halo、残差和候选；
6. 候选合法则提交，否则继续至局部上限。
