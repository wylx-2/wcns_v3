...
| S-V4 空间收敛 | 2D/3D、两 profile、24/48/96 三层网格 | 每组保存内部/边界/全局 L1/L2/Linf；PH 内部/壁带/全局 L2 阶分别 `>=3.5/0.55/1.0`，SCMM6 分别 `>=5.0/3.0/3.0` |
...
现有工程还没有闭式粘性制造源来保证高阶末态精确解，S 的空间矩阵直接对正弦温度扩散算子
作截断误差验证并输出三类原始表。设计前验确认 PH 内部保持约四阶，但当前等温壁闭合把全局
观测阶限制为约 1.1--1.4；SCMM6 壁带/全局分别约 3.1--3.9/3.6--3.9。上述分项下限在正式
验收前据实冻结，禁止用内部阶替代全局阶；PH 壁闭合是 S 的显式已知限制，不在本阶段改写
profile。时间阶矩阵采用固定网格的周期光滑解，以连续四档 CFL 的相邻末态差分隔离时间误差，
并保存所有 dt/误差数据。
