...
状态：**2026-09-11 本地全部冻结自动卡口通过；候选 `v1.1.0-rc.1` 可创建。按项目负责人
要求，在把 U 合入 `release/v1.1.0`、把 release 合入 `main` 或创建正式 `v1.1.0` 标签前停止，
等待人工核验。Linux/Clang/OpenMPI/Sanitizer 外部 CI 未实际运行，许可证决定仍待人工处理。**
...
本报告提交后只允许创建本地带注释标签 `v1.1.0-rc.1`。未收到新的明确人工批准前，不合并 U
到 `release/v1.1.0`，不合并 `main`，不创建正式 `v1.1.0` 标签。
