...
template <class Tag> class TaggedIndexRange3 {
public:
    ...
    [[nodiscard]] constexpr Index3 step() const { return untyped().step(); }

    [[nodiscard]] constexpr Extent3 counts() const { return untyped().counts(); }

    [[nodiscard]] constexpr std::size_t size() const { return untyped().size(); }

    [[nodiscard]] constexpr Index3 at(Index3 ordinal) const { return untyped().at(ordinal); }

    [[nodiscard]] constexpr IndexRange3 untyped() const { return {begin, end}; }

    friend constexpr bool operator==(const TaggedIndexRange3& lhs, const TaggedIndexRange3& rhs)
    {
