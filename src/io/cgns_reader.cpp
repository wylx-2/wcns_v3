...
std::vector<double> read_coordinate(int file,
                                    ...
                                    Index3 vertex_end)
{
    ...
    if (vertex_begin.i < 0 || vertex_begin.j < 0 || vertex_begin.k < 0
        ...
        || vertex_end.k < vertex_begin.k || vertex_end.i >= zone.vertex_extent.ni
        || vertex_end.j >= zone.vertex_extent.nj || vertex_end.k >= zone.vertex_extent.nk) {
        ...
        for (int column = 0; column < 3; ++column) {
            for (int inner = 0; inner < 3; ++inner) {
                result[static_cast<std::size_t>(row)][static_cast<std::size_t>(column)]
                    += lhs[static_cast<std::size_t>(row)][static_cast<std::size_t>(inner)]
                    * rhs[static_cast<std::size_t>(inner)][static_cast<std::size_t>(column)];
            }
        }
    }
    return result;
...
PeriodicTransform make_periodic_transform(const std::array<float, 3>& center,
                                          ...
                                          int physical_dimension)
{
    ...
    if (physical_dimension == 2 && (angle[0] != 0.0F || angle[1] != 0.0F)) {
        throw CgnsError("two-dimensional CGNS periodic rotation is ambiguous; "
    ...
    const RotationMatrix rx {{{{1.0, 0.0, 0.0}}, {{0.0, cx, -sx}}, {{0.0, sx, cx}}}};
    const RotationMatrix ry {{{{cy, 0.0, sy}}, {{0.0, 1.0, 0.0}}, {{-sy, 0.0, cy}}}};
    const RotationMatrix rz {{{{cz, -sz, 0.0}}, {{sz, cz, 0.0}}, {{0.0, 0.0, 1.0}}}};
    ...
    for (int component = 0; component < 3; ++component) {
        ...
        result.translation[index] = origin[index] - rotated_origin[index]
            + (component < physical_dimension ? static_cast<Real>(translation[index]) : 0.0);
    }
...
PeriodicTransform
read_periodic_transform(int file, const CgnsZoneMetadata& zone, int connection_index)
{
    std::array<float, 3> center {{}};
    std::array<float, 3> angle {{}};
    std::array<float, 3> translation {{}};
    const int status = cg_1to1_periodic_read(file,
                                             zone.base_file_index,
                                             zone.zone_file_index,
                                             connection_index,
                                             center.data(),
    ...
    return make_periodic_transform(center, angle, translation, zone.physical_dimension);
}

std::vector<double>
read_coordinate(int file, const CgnsZoneMetadata& zone, const char* coordinate_name)
{
    return read_coordinate(file,
                           zone,
                           coordinate_name,
                           {0, 0, 0},
                           {
        ...
        for (int j = 0; j < extent.nj; ++j) {
            for (int i = 0; i < extent.ni; ++i) {
                const auto source_index
                    = static_cast<std::size_t>((k * extent.nj + j) * extent.ni + i);
...
bool coordinate_exists(int file, const CgnsZoneMetadata& zone, const char* expected_name)
{
    ...
    check_cgns(cg_ncoords(file, zone.base_file_index, zone.zone_file_index, &coordinate_count),
               "cg_ncoords");
    for (int coordinate = 1; coordinate <= coordinate_count; ++coordinate) {
        ...
        check_cgns(
            cg_coord_info(
                file, zone.base_file_index, zone.zone_file_index, coordinate, &data_type, name),
            "cg_coord_info");
...
{
    switch (type) {
    case BCFarfield: return BoundaryType::Farfield;
    case BCInflow:
    case BCInflowSubsonic:
    case BCInflowSupersonic: return BoundaryType::Inflow;
    case BCOutflow:
    case BCOutflowSubsonic:
    case BCOutflowSupersonic: return BoundaryType::Outflow;
    case BCSymmetryPlane:
    case BCSymmetryPolar: return BoundaryType::Symmetry;
    case BCWall:
    case BCWallInviscid: return BoundaryType::SlipWall;
    case BCWallViscous: return BoundaryType::NoSlipAdiabaticWall;
    case BCWallViscousIsothermal: return BoundaryType::NoSlipIsothermalWall;
    default: return BoundaryType::Undefined;
    }
}

void validate_range_within_extent(const IndexRange3& range,
                                  Extent3 extent,
                                  int dimension,
                                  const char* label)
{
    ...
    for (int axis = 0; axis < dimension; ++axis) {
        ...
        if (axis_extent <= 0) {
            throw CgnsError(std::string(label) + " has a non-positive " + axis_names[axis_index]
                            + " extent");
        }

        const auto validate_endpoint = [&](int coordinate, const char* endpoint) {
            if (coordinate < 0 || coordinate >= axis_extent) {
                throw CgnsError(std::string(label) + " " + endpoint + " " + axis_names[axis_index]
                                + " index " + std::to_string(coordinate) + " is outside [0, "
                                + std::to_string(axis_extent - 1) + "]");
            }
        };
...
IndexRange3 convert_vertex_range(const std::vector<cgsize_t>& points,
                                 ...
                                 const char* label)
{
    ...
    for (int axis = 0; axis < cell_dimension; ++axis) {
        range.begin[static_cast<std::size_t>(axis)]
            = checked_dimension(points[static_cast<std::size_t>(axis)], label) - 1;
        range.end[static_cast<std::size_t>(axis)]
            = checked_dimension(points[static_cast<std::size_t>(cell_dimension + axis)], label) - 1;
    }
    if (cell_dimension == 2) {
        range.begin.k = 0;
        range.end.k = 0;
...
FaceLocation
identify_boundary_face(const IndexRange3& vertex_range, Extent3 vertex_extent, int cell_dimension)
{
    int normal_axis = -1;
...
IndexRange3 make_adjacent_cell_range(const IndexRange3& vertex_range,
                                     FaceLocation face,
                                     Extent3 cell_extent,
                                     int cell_dimension,
                                     const char* label)
    ...
    for (int axis = 0; axis < cell_dimension; ++axis) {
        if (axis == normal_axis) {
            const int coordinate
                = face.side == Side::Lower ? 0 : cell_extent[static_cast<std::size_t>(axis)] - 1;
...
IndexRange3 make_boundary_face_range(const IndexRange3& vertex_range,
                                     FaceLocation face,
                                     Extent3 cell_extent,
                                     int cell_dimension,
                                     const char* label)
{
    auto faces = make_adjacent_cell_range(vertex_range, face, cell_extent, cell_dimension, label);
    const auto normal_axis = static_cast<std::size_t>(face.axis);
    const int normal_coordinate = face.side == Side::Lower ? 0 : cell_extent[normal_axis];
...
std::vector<BoundaryPatch> read_boundaries(int file, const CgnsZoneMetadata& zone)
{
    int boundary_count = 0;
    check_cgns(cg_nbocos(file, zone.base_file_index, zone.zone_file_index, &boundary_count),
               "cg_nbocos");
    ...
    for (int boundary_index = 1; boundary_index <= boundary_count; ++boundary_index) {
        ...
        check_cgns(cg_boco_gridlocation_read(
                       file, zone.base_file_index, zone.zone_file_index, boundary_index, &location),
                   "cg_boco_gridlocation_read");
        if (location != Vertex) {
            throw CgnsError("only Vertex-located boundary PointRange is supported");
        }

        std::vector<cgsize_t> points(static_cast<std::size_t>(point_count * zone.cell_dimension));
        check_cgns(cg_boco_read(file,
                                zone.base_file_index,
                                zone.zone_file_index,
                                boundary_index,
                                points.data(),
        ...
        const auto vertex_range = convert_vertex_range(
            points, zone.vertex_extent, zone.cell_dimension, "boundary vertex PointRange");
        const auto face
            = identify_boundary_face(vertex_range, zone.vertex_extent, zone.cell_dimension);
        boundaries.push_back({
            name,
            convert_boundary_type(boundary_type),
            face,
            VertexRange(vertex_range),
            AdjacentCellRange(make_adjacent_cell_range(vertex_range,
                                                       face,
                                                       zone.cell_extent,
                                                       zone.cell_dimension,
                                                       "boundary adjacent-cell range")),
            BoundaryFaceRange(make_boundary_face_range(
                vertex_range, face, zone.cell_extent, zone.cell_dimension, "boundary face range")),
            {},
        });
    }
    return boundaries;
...
const CgnsZoneMetadata&
find_donor_zone(const CgnsMeshMetadata& metadata, int base_file_index, const char* donor_name)
{
    ...
    for (const auto& candidate : metadata.zones) {
        if (candidate.base_file_index == base_file_index && candidate.name == donor_name) {
            if (result != nullptr) {
                throw CgnsError(std::string("CGNS donor zone name is ambiguous: ") + donor_name);
            }
            result = &candidate;
        }
    }
...
std::vector<ConnectivityPatch> read_connectivities(int file,
                                                   const CgnsZoneMetadata& zone,
                                                   const CgnsMeshMetadata& metadata,
                                                   int ghost_width)
{
    int connection_count = 0;
    check_cgns(cg_n1to1(file, zone.base_file_index, zone.zone_file_index, &connection_count),
               "cg_n1to1");
    std::vector<ConnectivityPatch> connectivities;
    connectivities.reserve(static_cast<std::size_t>(connection_count));

    for (int connection_index = 1; connection_index <= connection_count; ++connection_index) {
        char name[33] = {};
        char donor_name[33] = {};
        std::array<cgsize_t, 6> receiver_points {};
        std::array<cgsize_t, 6> donor_points {};
        std::array<int, 3> raw_transform {{1, 2, 3}};
        check_cgns(cg_1to1_read(file,
                                zone.base_file_index,
                                zone.zone_file_index,
                                connection_index,
                                name,
        ...
        const auto& donor_zone = find_donor_zone(metadata, zone.base_file_index, donor_name);
        ...
        const std::vector<cgsize_t> receiver_values(receiver_points.begin(),
                                                    receiver_points.begin() + value_count);
        const std::vector<cgsize_t> donor_values(donor_points.begin(),
                                                 donor_points.begin() + value_count);
        const auto receiver_vertex_range
            = convert_vertex_range(receiver_values,
                                   zone.vertex_extent,
                                   zone.cell_dimension,
                                   "connectivity receiver vertex range");
        const auto donor_vertex_range = convert_vertex_range(donor_values,
        ...
        if (!transform.valid(zone.cell_dimension)) {
            throw CgnsError(std::string("connectivity ") + name + " has an invalid CGNS Transform");
        }
        if (transform.map(receiver_vertex_range.end,
        ...
        const auto periodic = read_periodic_transform(file, zone, connection_index);
        ...
        connectivities.push_back({
            ...
            ReceiverAdjacentCellRange(
                make_adjacent_cell_range(receiver_vertex_range,
                                         receiver_face,
                                         zone.cell_extent,
                                         zone.cell_dimension,
                                         "connectivity receiver adjacent-cell range")),
            DonorAdjacentCellRange(
                make_adjacent_cell_range(donor_vertex_range,
                                         donor_face,
                                         donor_zone.cell_extent,
                                         zone.cell_dimension,
                                         "connectivity donor adjacent-cell range")),
            SharedFaceRange(make_boundary_face_range(receiver_vertex_range,
                                                     receiver_face,
                                                     zone.cell_extent,
                                                     zone.cell_dimension,
...
StructuredBlock read_block_data(int file,
                                const CgnsZoneMetadata& zone,
                                const CgnsMeshMetadata& metadata,
                                RankId owner_rank,
                                int ghost_width)
{
    StructuredBlock block(zone.block_id,
                          zone.name,
                          owner_rank,
                          zone.cell_dimension,
    ...
    block.connectivities = read_connectivities(file, zone, metadata, ghost_width);
    return block;
}

const CgnsZoneMetadata& find_source_zone(const CgnsMeshMetadata& metadata, BlockId source_zone)
{
    for (const auto& zone : metadata.zones) {
        if (zone.block_id == source_zone) return zone;
    }
    throw CgnsError("partition leaf references unknown source zone " + std::to_string(source_zone));
}

Extent3 leaf_cell_extent(const CgnsPartitionLeaf& leaf, int dimension)
{
    return {
...
void validate_leaf(const CgnsPartitionLeaf& leaf, const CgnsZoneMetadata& zone)
{
    if (leaf.block_id < 0 || leaf.owner_rank < 0 || leaf.cell_begin.i < 0 || leaf.cell_begin.j < 0
        || leaf.cell_end.i <= leaf.cell_begin.i || leaf.cell_end.j <= leaf.cell_begin.j
        || leaf.cell_end.i > zone.cell_extent.ni || leaf.cell_end.j > zone.cell_extent.nj) {
...
StructuredBlock make_leaf_block(int file,
                                const CgnsZoneMetadata& zone,
                                ...
                                bool read_coordinates)
{
    ...
    StructuredBlock block(leaf.block_id,
    ...
    copy_coordinate(read_coordinate(file, zone, "CoordinateX", leaf.cell_begin, vertex_end),
                    block.coordinates.x);
    copy_coordinate(read_coordinate(file, zone, "CoordinateY", leaf.cell_begin, vertex_end),
                    block.coordinates.y);
    if (coordinate_exists(file, zone, "CoordinateZ")) {
        copy_coordinate(read_coordinate(file, zone, "CoordinateZ", leaf.cell_begin, vertex_end),
                        block.coordinates.z);
    } else if (zone.physical_dimension == 3) {
        throw CgnsError("three-dimensional physical space requires CoordinateZ");
    } else {
...
std::optional<IndexRange3>
intersect_leaf_vertices(const IndexRange3& range, const CgnsPartitionLeaf& leaf, int dimension)
{
    IndexRange3 result;
    for (int axis = 0; axis < dimension; ++axis) {
...
IndexRange3 localize_range(IndexRange3 range, const CgnsPartitionLeaf& leaf, int dimension)
{
    for (int axis = 0; axis < dimension; ++axis) {
        const auto a = static_cast<std::size_t>(axis);
...
bool is_face_range(const IndexRange3& range, FaceLocation face, int dimension)
{
    for (int axis = 0; axis < dimension; ++axis) {
...
std::vector<BoundaryPatch> slice_boundaries(const std::vector<BoundaryPatch>& source,
                                            const CgnsPartitionLeaf& leaf,
                                            Extent3 local_cells,
                                            int dimension)
{
    ...
    for (const auto& patch : source) {
        const auto intersection
            = intersect_leaf_vertices(patch.vertex_range.untyped(), leaf, dimension);
        ...
        result.push_back({
            ...
            AdjacentCellRange(make_adjacent_cell_range(local_vertices,
                                                       patch.face,
                                                       local_cells,
                                                       dimension,
                                                       "partitioned boundary adjacent-cell range")),
            BoundaryFaceRange(make_boundary_face_range(local_vertices,
                                                       patch.face,
                                                       local_cells,
                                                       dimension,
    ...
    const std::vector<CgnsPartitionLeaf>& leaves)
{
    ...
    for (const auto& receiver_leaf : leaves) {
        ...
        const auto connection_iterator = source_connections.find(receiver_leaf.source_zone);
        if (connection_iterator == source_connections.end()) continue;
        for (const auto& source : connection_iterator->second) {
            const auto receiver_intersection
                = intersect_leaf_vertices(source.receiver_vertex_range.untyped(),
                                          receiver_leaf,
                                          receiver_block.cell_dimension());
            if (!receiver_intersection
                || !is_face_range(*receiver_intersection,
                                  source.receiver_face,
                                  receiver_block.cell_dimension())) {
                continue;
            }
            const IndexRange3 donor_candidate {
                source.transform.map(receiver_intersection->begin,
                                     source.receiver_vertex_range.begin,
                                     source.donor_vertex_range.begin,
                                     receiver_block.cell_dimension()),
                source.transform.map(receiver_intersection->end,
                                     source.receiver_vertex_range.begin,
                                     source.donor_vertex_range.begin,
                                     receiver_block.cell_dimension()),
            };
            for (const auto& donor_leaf : leaves) {
                ...
                const auto donor_intersection = intersect_leaf_vertices(
                    donor_candidate, donor_leaf, receiver_block.cell_dimension());
                if (!donor_intersection
                    || !is_face_range(
                        *donor_intersection, source.donor_face, receiver_block.cell_dimension())) {
                    continue;
                }
                const auto inverse = source.transform.inverse(receiver_block.cell_dimension());
                const IndexRange3 receiver_piece {
                    inverse.map(donor_intersection->begin,
                                source.donor_vertex_range.begin,
                                source.receiver_vertex_range.begin,
                                receiver_block.cell_dimension()),
                    inverse.map(donor_intersection->end,
                                source.donor_vertex_range.begin,
                                source.receiver_vertex_range.begin,
                                receiver_block.cell_dimension()),
                };
                const auto local_receiver = localize_range(
                    receiver_piece, receiver_leaf, receiver_block.cell_dimension());
                const auto local_donor = localize_range(
                    *donor_intersection, donor_leaf, receiver_block.cell_dimension());
                const auto& donor_block = blocks.at(block_index.at(donor_leaf.block_id));
                receiver_block.connectivities.push_back({
                    source.name + "__part_" + std::to_string(receiver_leaf.block_id) + "_to_"
                        + std::to_string(donor_leaf.block_id),
...
void append_sibling_connectivities(std::vector<StructuredBlock>& blocks,
                                   const std::vector<CgnsPartitionLeaf>& leaves)
{
    std::unordered_map<BlockId, std::size_t> block_index;
    for (std::size_t index = 0; index < blocks.size(); ++index) {
    ...
    for (std::size_t first = 0; first < leaves.size(); ++first) {
        for (std::size_t second = first + 1; second < leaves.size(); ++second) {
            ...
            const FaceLocation lhs_face {static_cast<Axis>(normal_axis), lhs_side};
            const FaceLocation rhs_face {static_cast<Axis>(normal_axis),
                                         lhs_side == Side::Upper ? Side::Lower : Side::Upper};
            const auto lhs_vertices = localize_range(global_vertices, lhs, dimension);
            const auto rhs_vertices = localize_range(global_vertices, rhs, dimension);
            const auto add = [&](StructuredBlock& receiver,
                ...
                receiver.connectivities.push_back({
                    "partition_internal_" + std::to_string(receiver_leaf.block_id) + "_to_"
                        + std::to_string(donor_leaf.block_id),
                    receiver_leaf.block_id,
                    donor_leaf.block_id,
                    donor_leaf.owner_rank,
                    ...
                    ReceiverAdjacentCellRange(
                        make_adjacent_cell_range(receiver_vertices,
                                                 receiver_face,
                                                 receiver.cell_extent(),
                                                 dimension,
                                                 "partition sibling receiver cells")),
                    DonorAdjacentCellRange(
                        make_adjacent_cell_range(donor_vertices,
                                                 donor_face,
                                                 donor.cell_extent(),
                                                 dimension,
                                                 "partition sibling donor cells")),
                    SharedFaceRange(make_boundary_face_range(receiver_vertices,
                                                             receiver_face,
                                                             receiver.cell_extent(),
                                                             dimension,
            ...
            add(lhs_block, lhs, lhs_vertices, lhs_face, rhs_block, rhs, rhs_vertices, rhs_face);
            add(rhs_block, rhs, rhs_vertices, rhs_face, lhs_block, lhs, lhs_vertices, lhs_face);
        }
    }
}

void copy_topology_to_local(const std::vector<StructuredBlock>& global_blocks,
                            std::vector<StructuredBlock>& local_blocks)
{
    ...
    for (const auto& block : global_blocks)
        sources.emplace(block.id(), &block);
...
CgnsMeshMetadata CgnsReader::read_metadata(const std::string& path) const
{
    ...
    for (int base_index = 1; base_index <= base_count; ++base_index) {
        ...
        check_cgns(
            cg_base_read(file.id(), base_index, base_name, &cell_dimension, &physical_dimension),
            "cg_base_read");
        validate_base_dimensions(cell_dimension, physical_dimension);
        metadata.bases.push_back({base_index, base_name, cell_dimension, physical_dimension});
        ...
        for (int zone_index = 1; zone_index <= zone_count; ++zone_index) {
            ...
            check_cgns(cg_zone_type(file.id(), base_index, zone_index, &zone_type), "cg_zone_type");
            ...
            check_cgns(cg_zone_read(file.id(), base_index, zone_index, zone_name, size.data()),
                       "cg_zone_read");
...
StructuredBlock CgnsReader::read_block(const std::string& path,
                                       const CgnsZoneMetadata& zone,
                                       RankId owner_rank,
...
StructuredMesh
CgnsReader::read_mesh(const std::string& path, RankId owner_rank, int ghost_width) const
{
    const auto metadata = read_metadata(path);
    FileHandle file(path);
    std::vector<StructuredBlock> blocks;
    ...
    for (const auto& zone : metadata.zones) {
        blocks.push_back(read_block_data(file.id(), zone, metadata, owner_rank, ghost_width));
    }
...
CgnsPartitionedMesh CgnsReader::read_partitioned_mesh(const std::string& path,
                                                      const std::vector<CgnsPartitionLeaf>& leaves,
                                                      RankId local_rank,
                                                      int ghost_width) const
{
    ...
    for (const auto& zone : metadata.zones) {
        source_boundaries.emplace(zone.block_id, read_boundaries(file.id(), zone));
        source_connections.emplace(zone.block_id,
    ...
    for (const auto& leaf : leaves) {
        ...
        auto global = make_leaf_block(file.id(), zone, leaf, ghost_width, false);
        global.boundaries = slice_boundaries(
            source_boundaries.at(zone.block_id), leaf, global.cell_extent(), zone.cell_dimension);
        global_blocks.push_back(std::move(global));
        if (leaf.owner_rank == local_rank) {
            local_blocks.push_back(make_leaf_block(file.id(), zone, leaf, ghost_width, true));
        }
    }

    append_original_connectivities(global_blocks, source_connections, leaves);
    ...
    try {
        ...
        throw CgnsError(std::string("invalid partitioned CGNS topology: ") + error.what());
    }
