...
bool PeriodicTransform::valid(int dimension) const
{
    ...
    for (int row = 0; row < 3; ++row) {
        for (int column = 0; column < 3; ++column) {
            if (!std::isfinite(
                    rotation[static_cast<std::size_t>(row)][static_cast<std::size_t>(column)])) {
                return false;
            }
            Real product = 0.0;
            for (int inner = 0; inner < 3; ++inner) {
                product += rotation[static_cast<std::size_t>(row)][static_cast<std::size_t>(inner)]
                    * rotation[static_cast<std::size_t>(column)][static_cast<std::size_t>(inner)];
            }
    ...
    if (dimension == 2
        && (std::abs(rotation[0][2]) > tolerance || std::abs(rotation[1][2]) > tolerance
            || std::abs(rotation[2][0]) > tolerance || std::abs(rotation[2][1]) > tolerance
...
std::array<Real, 3> PeriodicTransform::apply_point(const std::array<Real, 3>& point) const
{
...
std::array<Real, 3> PeriodicTransform::apply_vector(const std::array<Real, 3>& vector) const
{
...
PeriodicTransform PeriodicTransform::inverse() const
{
    ...
    for (int row = 0; row < 3; ++row) {
        for (int column = 0; column < 3; ++column) {
            result.rotation[static_cast<std::size_t>(row)][static_cast<std::size_t>(column)]
                = rotation[static_cast<std::size_t>(column)][static_cast<std::size_t>(row)];
        }
    }
