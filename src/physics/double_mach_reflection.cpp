...
void require_finite(Real value, const char* label)
{
    if (!std::isfinite(value)) {
        throw PhysicsConfigurationError(std::string("double-Mach-reflection ") + label
                                        + " must be finite");
    }
}
...
void DoubleMachReflection::validate(Real gamma, int dimension) const
{
    ...
    if (!std::isfinite(gamma)
        || std::abs(gamma - classic_gamma) > 64.0 * std::numeric_limits<Real>::epsilon()) {
        throw PhysicsConfigurationError("classical double-Mach reflection requires gas.gamma=1.4");
    }
    if (dimension != 2) {
...
Real DoubleMachReflection::shock_x(Real y, Real time) const
{
    ...
    if (time < 0.0) {
        throw PhysicsConfigurationError("double-Mach-reflection time must be non-negative");
    }
...
PressurePrimitiveState DoubleMachReflection::exact_state(Real x, Real y, Real time) const
{
    return is_post_shock(x, y, time) ? post_shock_state() : upstream_state();
}
