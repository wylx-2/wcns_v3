    ...
    RankId root = 0)
{
    ...
    status = mpi.broadcast_string(std::move(status), root);
    if (status.rfind("ERROR\n", 0) == 0) {
        throw std::runtime_error(status.substr(6));
    }
    if (status != "OK") {
        throw std::runtime_error("invalid collective root-output status");
    }
}
    ...
    RankId root = 0)
{
    ...
    for (const auto& leaf : partition.leaves()) {
        ...
        const Array3D<Real>* values = &coordinates.x;
        if (component == ZoneComponent::Y) values = &coordinates.y;
        if (component == ZoneComponent::Z) values = &coordinates.z;
        const Real scale = context.dimensional ? context.reference.length() : 1.0;
        const auto extent = block.cell_extent();
        for (int k = 0; k < extent.nk; ++k) {
            for (int j = 0; j < extent.nj; ++j) {
                for (int i = 0; i < extent.ni; ++i) {
                    local_payload.push_back(scale * (*values)(i, j, k));
                }
            }
        }
    }
    ...
    if (status != "OK") {
        throw std::runtime_error("invalid chunked field-gather status");
    }
    return gathered;
}
    ...
    const std::string& path) const
{
    ...
    CgnsReader reader;
    std::unique_ptr<CgnsMeshMetadata> metadata;
    ...
    const std::string& path) const
{
    ...
    for (const auto& zone : partition_.zones()) {
        ...
        if (zone.cell_dimension == 3) write_component(zone, ZoneComponent::Z);
        for (const auto& name : config_.output.field.quantities) {
            write_component(zone, ZoneComponent::Quantity, name);
        }
    }
    collective_root_action(mpi_, [&] {
        output->close();
