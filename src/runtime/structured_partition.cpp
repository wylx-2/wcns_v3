...
bool candidate_less(const SplitCandidate& lhs, const SplitCandidate& rhs)
{
    return std::make_tuple(
               lhs.largest_child, lhs.interface_cells, -lhs.axis_length, lhs.axis, lhs.cut)
        < std::make_tuple(
               rhs.largest_child, rhs.interface_cells, -rhs.axis_length, rhs.axis, rhs.cut);
}

SplitCandidate best_split(const PartitionLeaf& leaf, std::size_t leaf_index, int minimum_cells)
{
    SplitCandidate best;
...
std::size_t maximum_leaves_for_zone(const PartitionZone& zone, int minimum_cells)
{
    std::size_t result = 1;
    for (int axis = 0; axis < zone.cell_dimension; ++axis) {
        const int chunks = zone.cell_extent[static_cast<std::size_t>(axis)] / minimum_cells;
        if (chunks <= 0) return 0;
        if (result > std::numeric_limits<std::size_t>::max() / static_cast<std::size_t>(chunks)) {
...
Real load_ratio(const BlockDistribution& distribution)
{
    ...
    const auto total = std::accumulate(loads.begin(), loads.end(), static_cast<std::size_t>(0));
...
{
    std::sort(leaves.begin(), leaves.end(), [](const PartitionLeaf& lhs, const PartitionLeaf& rhs) {
        return std::tie(lhs.source_zone,
                        lhs.cells.begin.i,
                        lhs.cells.begin.j,
                        lhs.cells.begin.k,
                        lhs.cells.end.i,
...
void split_leaf(std::vector<PartitionLeaf>& leaves, const SplitCandidate& split)
{
...
SplitCandidate choose_split(const std::vector<PartitionLeaf>& leaves, int minimum_cells)
{
...
bool CellBox::valid(int dimension) const
{
    ...
    return dimension == 3 ? end.k > begin.k : begin.k == 0 && end.k == 1;
}
...
StructuredPartitionPlan StructuredPartitionPlan::build(std::vector<PartitionZone> zones,
                                                       int rank_count,
                                                       const PartitionConfig& config)
{
    ...
    std::sort(zones.begin(), zones.end(), [](const PartitionZone& lhs, const PartitionZone& rhs) {
    ...
    for (std::size_t index = 0; index < zones.size(); ++index) {
        ...
        if (zone.source_zone < 0 || zone.name.empty()
            || (zone.cell_dimension != 2 && zone.cell_dimension != 3) || zone.cell_extent.ni <= 0
            || zone.cell_extent.nj <= 0
            || (zone.cell_dimension == 3 ? zone.cell_extent.nk <= 0 : zone.cell_extent.nk != 1)) {
    ...
    for (const auto& zone : result.zones_) {
        const auto feasible = maximum_leaves_for_zone(zone, config.min_cells_per_active_direction);
        if (feasible == 0) {
            throw CaseConfigurationError("zone " + zone.name
    ...
    if (config.mode == PartitionMode::ZonesOnly && !config.allow_idle_ranks
        && result.leaves_.size() < static_cast<std::size_t>(rank_count)) {
        throw CaseConfigurationError("zones_only partition has fewer zones than ranks");
    }
    if (!config.allow_idle_ranks
        && result.maximum_feasible_leaf_count_ < static_cast<std::size_t>(rank_count)) {
        throw CaseConfigurationError("requested ranks exceed maximum feasible leaves: "
                                     + std::to_string(result.maximum_feasible_leaf_count_));
    }

    const auto required_leaves = config.mode == PartitionMode::ZonesOnly
        ? result.leaves_.size()
        : std::min(static_cast<std::size_t>(rank_count), result.maximum_feasible_leaf_count_);
    while (result.leaves_.size() < required_leaves) {
        const auto split = choose_split(result.leaves_, config.min_cells_per_active_direction);
        if (!split.valid) break;
        split_leaf(result.leaves_, split);
    }

    if (config.mode != PartitionMode::ZonesOnly) {
        const auto leaf_limit
            = std::min(result.maximum_feasible_leaf_count_,
                       std::max(required_leaves, static_cast<std::size_t>(rank_count) * 4));
        while (result.leaves_.size() < leaf_limit) {
            assign_stable_ids(result.leaves_);
            const auto trial = BlockDistribution::balanced(leaf_loads(result.leaves_), rank_count);
    ...
    assign_stable_ids(result.leaves_);
    result.distribution_ = BlockDistribution::balanced(leaf_loads(result.leaves_), rank_count);
...
void StructuredPartitionPlan::validate() const
{
    ...
    for (const auto& zone : zones_)
        zones.emplace(zone.source_zone, &zone);
    ...
    for (const auto& zone : zones_) {
        if (covered[zone.source_zone] != zone.cell_extent.size()) {
            throw CaseConfigurationError("partition leaves do not exactly cover source zone "
                                         + zone.name);
        }
    }
}
...
std::string StructuredPartitionPlan::summary() const
{
    ...
    for (const auto& leaf : leaves_) {
        result << ";leaf=" << leaf.block << ",zone=" << leaf.source_zone << ",range=["
               << leaf.cells.begin.i << ',' << leaf.cells.begin.j << ',' << leaf.cells.begin.k
               << ":" << leaf.cells.end.i << ',' << leaf.cells.end.j << ',' << leaf.cells.end.k
               << "),owner=" << leaf.owner << ",generation=" << leaf.generation;
    }
    result << ')';
    return result.str();
}
