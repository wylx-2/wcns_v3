        ...
        for (int j = 0; j < extent.nj; ++j) {
            for (int i = 0; i < extent.ni; ++i) {
                for (int component = 0; component < euler_components; ++component) {
                    result[offset++] = block.flow.conservative(i, j, k, component);
                }
            }
        }
    }
...
void update_stage(StructuredBlock& block,
...
void SsprkWorkspace::prepare(const std::vector<StructuredBlock*>& blocks)
{
...
void advance_ssprk3(const std::vector<StructuredBlock*>& blocks,
                    Real time_step,
                    ...
                    const ResidualEvaluator& evaluate_residuals)
{
    ...
    evaluate_residuals();
    for (std::size_t b = 0; b < blocks.size(); ++b) {
        update_stage(*blocks[b], workspace.initial_[b], 0.75, 0.25, 0.25 * time_step);
    }

    evaluate_residuals();
    for (std::size_t b = 0; b < blocks.size(); ++b) {
        update_stage(
            *blocks[b], workspace.initial_[b], 1.0 / 3.0, 2.0 / 3.0, 2.0 * time_step / 3.0);
    }
}
...
{
    SsprkWorkspace workspace;
    advance_ssprk3(blocks, workspace, time_step, initial_time, evaluate_residuals);
}
                    ...
                    const TimedResidualEvaluator& evaluate_residuals)
{
    ...
    if (!std::isfinite(time_step) || time_step <= 0.0 || !std::isfinite(initial_time)) {
