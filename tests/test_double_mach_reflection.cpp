...
wcns::StructuredBlock double_mach_top_block()
{
    ...
    block.boundaries.push_back({
        "top", BoundaryType::DoubleMachReflection,
        {Axis::J, Side::Upper},
        {{0, 5, 0}, {6, 5, 0}},
        {{0, 4, 0}, {5, 4, 0}},
        {{0, 5, 0}, {5, 5, 0}}, {}});
...
void test_double_mach_reflection()
{
    ...
    WCNS_REQUIRE_THROWS(
        ...
        model.validate(double_mach_gas(1.3).gamma(), 2));
    ...
    WCNS_REQUIRE_NEAR(model.shock_x(1.0, 0.2),
        1.0 / 6.0 + 5.0 / sqrt_three, 1.0e-14);
    ...
    const auto initialized_post = pressure_primitive(
        FlowInitializer::evaluate(
            initial, {0.1, 0.0, 0.0}, gas, reference, floors, 2),
        gas, reference, floors, 2);
    const auto initialized_upstream = pressure_primitive(
        FlowInitializer::evaluate(
            initial, {3.0, 1.0, 0.0}, gas, reference, floors, 2),
        gas, reference, floors, 2);
    ...
    BoundaryPatch bottom {
        "bottom", BoundaryType::DoubleMachReflection,
        {Axis::J, Side::Lower}, {}, {}, {}, {}};
    const auto bottom_inflow = apply_inviscid_boundary_face_state(
        bottom, interior, reconstructed, {0.0, -1.0, 0.0}, data,
        {}, gas, reference, floors, 2, {0.1, 0.0, 0.0}, 0.0);
    WCNS_REQUIRE(bottom_inflow == downstream);
    const auto bottom_wall = apply_inviscid_boundary_face_state(
        bottom, interior, reconstructed, {0.0, -1.0, 0.0}, data,
        {}, gas, reference, floors, 2, {0.5, 0.0, 0.0}, 0.0);
    ...
    const auto top_initial = apply_inviscid_boundary_face_state(
        top, interior, reconstructed, {0.0, 1.0, 0.0}, data,
        {}, gas, reference, floors, 2, {1.0, 1.0, 0.0}, 0.0);
    const auto top_later = apply_inviscid_boundary_face_state(
        top, interior, reconstructed, {0.0, 1.0, 0.0}, data,
        {}, gas, reference, floors, 2, {1.0, 1.0, 0.0}, 0.1);
    WCNS_REQUIRE(top_initial == upstream);
    WCNS_REQUIRE(top_later == downstream);

    auto block = double_mach_top_block();
    const auto upstream_temperature = temperature_primitive(
        upstream, gas, reference, floors, 2);
    const auto conservative = thermodynamic_conservative(
        upstream_temperature, gas, reference, floors, 2);
    ...
    const auto first_fill = PhysicalGhostStateOperator::fill(
        block, boundary_data, gas, reference, floors, 1, 0.0);
    ...
    const auto second_fill = PhysicalGhostStateOperator::fill(
        block, boundary_data, gas, reference, floors, 2, 0.3);
