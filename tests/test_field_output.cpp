...
void validate_cgns(const std::string& path)
{
    ...
    check_cgns(cg_field_read(file, 1, 1, 1, "Density", RealDouble, lower, upper, values.data()),
               "cg_field_read Density");
    for (const double value : values)
        WCNS_REQUIRE_NEAR(value, 1.0, 1.0e-12);
    check_cgns(cg_field_read(file, 1, 1, 1, "VelocityX", RealDouble, lower, upper, values.data()),
               "cg_field_read VelocityX");
    for (const double value : values)
        WCNS_REQUIRE_NEAR(value, 0.2, 1.0e-12);
...
{
    if (argc != 4) {
        std::cerr << "usage: wcns_field_output_tests <output-directory> "
    ...
    try {
        ...
        const std::string stem = "output-freestream.field.step00000000.time0p000000000eP00";
        ...
        const auto history = read_text(join(directory,
                                            "output-freestream.history.r" + ranks
        ...
        const auto statistics
            = read_text(join(directory, "output-freestream.statistics.r" + ranks + ".dat"));
