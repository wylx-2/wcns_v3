...
wcns::Real synthetic_flux(wcns::BlockId block, wcns::Axis axis, wcns::Index3 index, int component)
{
    return 10000.0 * static_cast<wcns::Real>(block)
        + 1000.0 * static_cast<wcns::Real>(static_cast<int>(axis)) + 100.0 * index.i
        + 10.0 * index.j + index.k + 0.01 * component;
}

wcns::ConservativeState expected_received(const wcns::FaceFluxExchangeDescriptor& descriptor,
                                          const wcns::FaceFluxPair& pair)
{
    wcns::ConservativeState donor {};
    for (int component = 0; component < wcns::euler_components; ++component) {
        donor[static_cast<std::size_t>(component)]
            = synthetic_flux(descriptor.donor_block, descriptor.donor_axis, pair.donor, component);
    }
    const auto momentum
        = descriptor.periodic.inverse().apply_vector({{donor[1], donor[2], donor[3]}});
    return {
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        for (const auto& exchange : plan.exchanges()) {
            ...
            valid = valid && exchange.donor_rank == distribution.owner(exchange.donor_block);
            valid = valid && exchange.shared_face_owner == 0;
        }
        WCNS_REQUIRE(mpi.all_true(valid));

        const std::uint64_t flux_version = 11;
        const auto flux_plan = FaceFluxHaloPlan::build(mesh, profile, flux_version);
        ...
        for (const auto& block : mesh.blocks()) {
            ...
            auto [iterator, inserted] = flux_fields.emplace(
                ...
                std::forward_as_tuple(
                    block.cell_extent(), block.cell_dimension(), profile.kind(), flux_version));
                        ...
                        for (int i = 0; i < extent.ni; ++i) {
                            for (int component = 0; component < euler_components; ++component) {
                                field(i, j, k, component)
                                    = synthetic_flux(block.id(), axis, {i, j, k}, component);
                            }
                        }
                    }
                }
        ...
        for (const auto& descriptor : flux_plan.exchanges()) {
            ...
            const auto& receiver
                = registry.field(descriptor.receiver_block).field(descriptor.receiver_axis);
            for (const auto& pair : descriptor.pairs) {
                ...
                for (int component = 0; component < euler_components; ++component) {
                    flux_valid = flux_valid
                        && std::abs(
                               receiver(
                                   pair.receiver.i, pair.receiver.j, pair.receiver.k, component)
