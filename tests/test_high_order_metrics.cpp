...
WarpedMetricError warped_metric_error(wcns::AlgorithmProfileKind kind, int cell_count)
{
    using namespace wcns;
    StructuredBlock block(
        10 + cell_count, "warped", 0, 2, 2, {cell_count + 1, cell_count + 1, 1}, 3);
    ...
    for (int j = margin; j < cell_count - margin; ++j) {
        for (int i = margin; i < cell_count - margin; ++i) {
            ...
            const Real x_s = 1.0 + amplitude * two_pi * std::cos(two_pi * s) * std::sin(two_pi * t);
            const Real x_t = amplitude * two_pi * std::sin(two_pi * s) * std::cos(two_pi * t);
            const Real y_s = -amplitude * two_pi * std::sin(two_pi * s) * std::sin(two_pi * t);
            const Real y_t = 1.0 + amplitude * two_pi * std::cos(two_pi * s) * std::cos(two_pi * t);
            const Real exact = (x_s * y_t - x_t * y_s) / static_cast<Real>(cell_count * cell_count);
    ...
    for (int component = 0; component < 3; ++component) {
        ...
        std::vector<std::vector<Real>> di(static_cast<std::size_t>(cell_count),
        ...
        for (int i = 0; i < cell_count; ++i) {
            ...
            for (int j = 0; j < cell_count; ++j) {
                identity_max
                    = std::max(identity_max,
                               std::abs(di[static_cast<std::size_t>(j)][static_cast<std::size_t>(i)]
...
void test_phenglei_high_order_metrics()
{
    ...
    for (int j = 0; j < cells.nj; ++j) {
        for (int i = 0; i < cells.ni; ++i) {
            ...
            WCNS_REQUIRE_NEAR(result.metric.cell_coordinates().x(i, j, 0),
                              2.0 * (i + 0.5) + 0.5 * (j + 0.5) + 1.0,
                              2.0e-14);
            WCNS_REQUIRE_NEAR(
                result.diagnostics.reference_volume(i, j, 0), exact_jacobian, 2.0e-14);
        }
    }
    for (int j = 0; j < cells.nj; ++j) {
        for (int i = 0; i <= cells.ni; ++i) {
    ...
    WCNS_REQUIRE_THROWS(GeometryError, initialize_metric_field(too_small, profile));
}
...
void test_phenglei_high_order_metrics_3d()
{
    ...
    const auto extracted = extract_metric_field(result.metric, {1, 1, 1}, {2, 2, 2});
    ...
    WCNS_REQUIRE(payload.size() == metric_field_payload_size({2, 2, 2}, 3));
    const auto unpacked
        = unpack_metric_field(AlgorithmProfileKind::PhengleiWcns, {2, 2, 2}, 3, payload);
...
void test_scmm6_high_order_metrics()
{
    ...
    for (int j = 0; j < cells.nj; ++j) {
        for (int i = 0; i < cells.ni; ++i) {
            ...
            WCNS_REQUIRE_NEAR(result.metric.cell_coordinates().y(i, j, 0),
    ...
    WCNS_REQUIRE_THROWS(ProfileError, initialize_metric_field(block, profile, invalid_fallback));
...
{
    using namespace wcns;
    const auto ph_coarse = warped_metric_error(AlgorithmProfileKind::PhengleiWcns, 16);
    const auto ph_fine = warped_metric_error(AlgorithmProfileKind::PhengleiWcns, 32);
    ...
    const auto scmm_coarse = warped_metric_error(AlgorithmProfileKind::Scmm6Wcns, 16);
    const auto scmm_fine = warped_metric_error(AlgorithmProfileKind::Scmm6Wcns, 32);
