...
public:
    ConstantFieldQuantity(std::string name, std::vector<std::string> dependencies, wcns::Real value)
        : value_(value)
    {
    ...
    const wcns::QuantityDescriptor& descriptor() const override { return descriptor_; }

    wcns::Real evaluate_cell(const wcns::StructuredBlock&,
...
public:
    explicit ConstantStatisticQuantity(std::string name) { descriptor_.name = std::move(name); }

    const wcns::QuantityDescriptor& descriptor() const override { return descriptor_; }

    wcns::Real evaluate(const wcns::StatisticContext&) const override { return 7.0; }
...
void test_quantity_registry()
{
    ...
    const auto profile = wcns::ProfileFactory::create(wcns::AlgorithmProfileKind::PhengleiWcns);
    ...
    wcns::FlowInitializer::initialize_block(block, metric, initial, gas, reference);
    ...
    const auto dimensional_j = registry.evaluate("jacobian", block, metric, context);
    ...
    WCNS_REQUIRE_THROWS(std::invalid_argument, registry.validate_selection({"rho", "missing"}));
    WCNS_REQUIRE_THROWS(std::invalid_argument, registry.validate_selection({"rho", "rho"}));
    ...
    const auto custom = registry.evaluate("custom_cell", block, metric, context);
    ...
    WCNS_REQUIRE_THROWS(std::invalid_argument, cyclic.validate_selection({"cycle_a"}));
    ...
    statistics.validate_selection(
        {"xz_mean_u_j0", "xz_mass_flow_x_j0", "xz_mean_u_j3", "xz_mass_flow_x_j3"});
    ...
    WCNS_REQUIRE_THROWS(std::invalid_argument, wcns::validate_xz_plane_statistics({4}, partition));
    ...
    WCNS_REQUIRE(yz_names
                 == std::vector<std::string>({"yz_mean_u_plane0",
    ...
    wcns::register_channel_wall_statistics(statistics, "bottom", "top", 1.0);
    statistics.validate_selection(wcns::channel_wall_statistic_names());
    WCNS_REQUIRE(wcns::channel_wall_statistic_names()
                 == std::vector<std::string>({"channel_wall_shear_lower",
                                              "channel_wall_shear_upper",
                                              "channel_wall_shear_mean",
                                              "channel_friction_velocity",
                                              "channel_re_tau"}));
    statistics.register_quantity(std::make_shared<ConstantStatisticQuantity>("custom_statistic"));
