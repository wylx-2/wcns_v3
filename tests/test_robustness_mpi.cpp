...
int main(int argc, char** argv)
{
    try {
        ...
        StructuredBlock block(
            mpi.rank(), "robust-retry", mpi.rank(), 2, 2, {5, 5, 1}, 3);
        ...
        const Real accepted = advance_ssprk3_with_robustness(
            mpi, blocks, topology, profile, FluxDifferenceMode::Profile,
            gas, reference, floors, config, ladder, 1.0, 0.0,
        ...
        WCNS_REQUIRE_NEAR(
            block.flow.conservative(0, 0, 0, density), 0.5, 1.0e-14);
