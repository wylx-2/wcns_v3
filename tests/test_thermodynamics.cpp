...
void test_thermodynamics()
{
    ...
    WCNS_REQUIRE_NEAR(reference.mach(), 340.0 / std::sqrt(1.4 * 287.0 * 300.0), 1.0e-15);
    ...
    WCNS_REQUIRE_THROWS(PhysicsConfigurationError, ReferenceScales::derive(forbidden_mach, gas));
    auto invalid_reference = input;
    invalid_reference.temperature = 0.0;
    WCNS_REQUIRE_THROWS(PhysicsConfigurationError, ReferenceScales::derive(invalid_reference, gas));
    ...
    WCNS_REQUIRE_THROWS(PhysicsConfigurationError, floors.jacobian_floor(0.0));
    ...
    WCNS_REQUIRE_THROWS(PhysicsConfigurationError, scaling.normalized_smoothness(-1.0, 0));
    WCNS_REQUIRE_THROWS(PhysicsConfigurationError,
                        scaling.normalized_smoothness(1.0, fluid_components));

    const TemperaturePrimitiveState temperature_state {{1.2, 0.3, -0.2, 0.0, 1.1}};
    const auto pressure_state = pressure_primitive(temperature_state, gas, reference, floors, 2);
    WCNS_REQUIRE_NEAR(
        pressure_state[4], 1.2 * 1.1 / (1.4 * reference.mach() * reference.mach()), 1.0e-14);
    const auto recovered_temperature
        = temperature_primitive(pressure_state, gas, reference, floors, 2);
    for (int component = 0; component < fluid_components; ++component) {
        WCNS_REQUIRE_NEAR(recovered_temperature[static_cast<std::size_t>(component)],
                          temperature_state[static_cast<std::size_t>(component)],
                          1.0e-14);
    }

    const auto conservative
        = thermodynamic_conservative(temperature_state, gas, reference, floors, 2);
    const auto recovered_conservative
        = temperature_primitive_from_conservative(conservative, gas, reference, floors, 2);
    for (int component = 0; component < fluid_components; ++component) {
        WCNS_REQUIRE_NEAR(recovered_conservative[static_cast<std::size_t>(component)],
                          temperature_state[static_cast<std::size_t>(component)],
                          1.0e-14);
    }
    WCNS_REQUIRE(thermodynamic_sound_speed(temperature_state, gas, reference, floors, 2) > 0.0);
    WCNS_REQUIRE(thermodynamic_total_enthalpy(temperature_state, gas, reference, floors, 2) > 0.0);
