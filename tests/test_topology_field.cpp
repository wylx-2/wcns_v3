...
void test_topology_field()
{
    ...
    const FieldDomain state_domain({4, 3, 1},
                                   3,
                                   2,
    ...
    WCNS_REQUIRE(state.at({-1, 1, 0}, 0, AccessRegion::PhysicalBoundarySlab) == 3.0);
    WCNS_REQUIRE_THROWS(std::out_of_range,
                        state.at({-1, -1, 0}, 0, AccessRegion::PhysicalBoundarySlab));
    WCNS_REQUIRE_THROWS(std::out_of_range, state.at({-1, 3, 0}, 0, AccessRegion::ConnectionHalo));
    WCNS_REQUIRE_THROWS(std::out_of_range,
                        state.at({1, 1, -1}, 0, AccessRegion::PhysicalBoundarySlab));
    WCNS_REQUIRE_THROWS(std::out_of_range, state.at({-1, 1, 0}, 0, AccessRegion::Interior));

    const FieldDomain geometry_domain({5, 4, 1},
                                      3,
                                      2,
