...
double parse_warp(const char* text)
{
    ...
    if (consumed != std::string(text).size() || !std::isfinite(value) || std::abs(value) >= 0.2) {
...
double clustered_unit_coordinate(double logical, double center, double strength)
{
    ...
    if (logical <= center) {
        return center * (1.0 - std::sinh(strength * (center - logical) / center) / denominator);
    }
    return center
        + (1.0 - center) * std::sinh(strength * (logical - center) / (1.0 - center)) / denominator;
}
...
double wall_clustered_unit_coordinate(double logical, double strength)
{
    ...
    return 0.5 * (1.0 + std::tanh(strength * (2.0 * logical - 1.0)) / std::tanh(strength));
}
                    ...
                    BCType_t type = BCFarfield)
{
    int boundary = 0;
    check_cgns(
        cg_boco_write(file, base, zone, name.c_str(), type, PointRange, 2, range.data(), &boundary),
...
std::vector<cgsize_t> face_range(int dimension, int i, int nj, int nk)
{
              ...
              double cluster_strength = 0.0)
{
    ...
    if (clustered
        && (!(cluster_x > 0.0 && cluster_x < length_x) || !(cluster_y > 0.0 && cluster_y < length_y)
    ...
    try {
        int base = 0;
        check_cgns(cg_base_write(file, "WCNSReleaseGrid", dimension, dimension, &base),
        ...
        for (int zone_index = 0; zone_index < zones_i; ++zone_index) {
            ...
            check_cgns(
                cg_zone_write(
                    file, base, zone_name(zone_index).c_str(), size.data(), Structured, &zone),
                "cg_zone_write release grid");

            const std::size_t count = static_cast<std::size_t>(ni) * static_cast<std::size_t>(nj)
            ...
            for (int k = 0; k < nk; ++k) {
                const double zeta
                    = dimension == 3 ? static_cast<double>(k) / static_cast<double>(cells_k) : 0.0;
                for (int j = 0; j < nj; ++j) {
                    const double eta = static_cast<double>(j) / static_cast<double>(cells_j);
                    for (int i = 0; i < ni; ++i) {
                        const int global_i = zone_index * local_cells_i + i;
                        const double xi
                            = static_cast<double>(global_i) / static_cast<double>(cells_i);
                        const double envelope = dimension == 3 ? std::sin(pi * zeta) : 1.0;
                        const auto index = static_cast<std::size_t>((k * nj + j) * ni + i);
                        const double base_x = clustered ? length_x
                                * clustered_unit_coordinate(xi,
                                                            cluster_x / length_x,
                                                            cluster_strength)
                                                        : length_x * xi;
                        const double base_y = clustered ? length_y
                                * clustered_unit_coordinate(eta,
                                                            cluster_y / length_y,
                                                            cluster_strength)
                                                        : length_y * eta;
                        x[index] = base_x
                            + length_x * warp * std::sin(pi * xi) * std::sin(pi * eta) * envelope;
                        y[index] = base_y;
            ...
            int coordinate = 0;
            check_cgns(
                cg_coord_write(file, base, zone, RealDouble, "CoordinateX", x.data(), &coordinate),
                "cg_coord_write release X");
            check_cgns(
                cg_coord_write(file, base, zone, RealDouble, "CoordinateY", y.data(), &coordinate),
                "cg_coord_write release Y");
            if (dimension == 3) {
                check_cgns(cg_coord_write(
                               file, base, zone, RealDouble, "CoordinateZ", z.data(), &coordinate),
            ...
            if (!periodic_x && zone_index == 0) {
                write_boundary(file, base, zone, "left", face_range(dimension, 1, nj, nk));
            }
            if (!periodic_x && zone_index + 1 == zones_i) {
                write_boundary(file, base, zone, "right", face_range(dimension, ni, nj, nk));
            }
        }
        ...
        for (int zone_index = 0; zone_index + 1 < zones_i; ++zone_index) {
            ...
            write_connection(file,
                             base,
                             left,
                             "to-" + zone_name(zone_index + 1),
        ...
        if (periodic_x) {
            const int forward = write_connection(file,
                                                 base,
                                                 1,
                                                 "periodic-left",
                                                 zone_name(zones_i - 1),
                                                 ...
                                                 face_range(dimension, ni, nj, nk),
                                                 dimension);
            const int reverse = write_connection(file,
                                                 base,
                                                 zones_i,
                                                 "periodic-right",
                                                 zone_name(0),
            ...
            check_cgns(cg_1to1_periodic_write(
                           file, base, 1, forward, center.data(), angle.data(), translation.data()),
                       "cg_1to1_periodic_write forward");
            translation[0] = -1.0F;
            check_cgns(
                cg_1to1_periodic_write(
                    file, base, zones_i, reverse, center.data(), angle.data(), translation.data()),
...
void generate_periodic_square(const std::string& path,
                              int cells_i,
                              int cells_j,
                              double length,
...
{
    if (cells_i % 2 != 0 || cells_j % 2 != 0 || !std::isfinite(length) || length <= 0.0) {
        throw std::invalid_argument(
            "periodic-square requires even cell counts and positive finite length");
    }
    if (!std::isfinite(x_warp_amplitude) || !std::isfinite(y_warp_amplitude)
        || x_warp_amplitude < 0.0 || y_warp_amplitude < 0.0) {
        throw std::invalid_argument(
            "periodic-square warp amplitudes must be finite and nonnegative");
    }
    constexpr double pi = 3.141592653589793238462643383279502884;
    const double maximum_cross_product
        = 8.0 * pi * pi * x_warp_amplitude * y_warp_amplitude / (length * length);
    ...
    try {
        int base = 0;
        check_cgns(cg_base_write(file, "WCNSPeriodicSquare", 2, 2, &base),
                   "cg_base_write periodic square");
        const auto square_zone_name
            = [](int i, int j) { return "Zone" + std::to_string(j * 2 + i + 1); };
        for (int zone_j = 0; zone_j < 2; ++zone_j) {
            for (int zone_i = 0; zone_i < 2; ++zone_i) {
                ...
                int zone = 0;
                check_cgns(cg_zone_write(file, base, name.c_str(), size, Structured, &zone),
                ...
                for (int j = 0; j < nj; ++j) {
                    for (int i = 0; i < ni; ++i) {
                        ...
                        const double xi = length * static_cast<double>(zone_i * local_i + i)
                            / static_cast<double>(cells_i);
                        const double eta = length * static_cast<double>(zone_j * local_j + j)
                        ...
                        x[index] = xi + x_warp_amplitude * std::sin(2.0 * pi * eta / length);
                        y[index] = eta + y_warp_amplitude * std::sin(4.0 * pi * xi / length);
                    }
                }
                int coordinate = 0;
                check_cgns(cg_coord_write(
            ...
            for (int zone_i = 0; zone_i < 2; ++zone_i) {
                ...
                const auto add = [&](const std::string& name,
                    ...
                    const int connection
                        = write_connection(file, base, zone, name, donor, range, donor_range, 2);
                    if (translation[0] != 0.0F || translation[1] != 0.0F) {
                        std::array<float, 3> center {{0.0F, 0.0F, 0.0F}};
                        std::array<float, 3> angle {{0.0F, 0.0F, 0.0F}};
                        check_cgns(cg_1to1_periodic_write(file,
                                                          base,
                                                          zone,
                                                          connection,
                                                          center.data(),
                ...
                add("imin",
                    square_zone_name(other_i, zone_j),
                    {1, 1, 1, nj},
                    {ni, 1, ni, nj},
                ...
                add("jmin",
                    square_zone_name(zone_i, other_j),
                    {1, 1, ni, 1},
                    {1, nj, ni, nj},
...
void generate_periodic_channel(const std::string& path,
                               int cells_i,
                               int cells_j,
                               int cells_k,
                               int zones_i,
                               ...
                               double origin_y = 0.0)
{
    if (cells_i % zones_i != 0 || cells_k % zones_k != 0 || zones_i < 2 || zones_k < 2
    ...
    try {
        int base = 0;
        check_cgns(cg_base_write(file, "WCNSPeriodicChannel", 3, 3, &base),
        ...
        for (int zone_k = 0; zone_k < zones_k; ++zone_k) {
            for (int zone_i = 0; zone_i < zones_i; ++zone_i) {
                ...
                const auto add = [&](const std::string& name,
                                     ...
                                     std::array<float, 3> translation) {
                    const int connection
                        = write_connection(file, base, zone, name, donor, range, donor_range, 3);
                ...
                add("imin",
                    channel_zone_name(previous_i, zone_k),
                    {1, 1, 1, 1, nj, nk},
                    {ni, 1, 1, ni, nj, nk},
                    {{zone_i == 0 ? static_cast<float>(length_x) : 0.0F, 0.0F, 0.0F}});
                add("imax",
                    channel_zone_name(next_i, zone_k),
                    {ni, 1, 1, ni, nj, nk},
                    {1, 1, 1, 1, nj, nk},
                    {{zone_i + 1 == zones_i ? -static_cast<float>(length_x) : 0.0F, 0.0F, 0.0F}});
                add("kmin",
                    channel_zone_name(zone_i, previous_k),
                    {1, 1, 1, ni, nj, 1},
                    {1, 1, nk, ni, nj, nk},
                    {{0.0F, 0.0F, zone_k == 0 ? static_cast<float>(length_z) : 0.0F}});
                add("kmax",
                    channel_zone_name(zone_i, next_k),
                    {1, 1, nk, ni, nj, nk},
                    {1, 1, 1, ni, nj, 1},
                    {{0.0F, 0.0F, zone_k + 1 == zones_k ? -static_cast<float>(length_z) : 0.0F}});
            }
        }
...
void generate_cylinder_o_grid(const std::string& path,
                              int cells_theta,
                              int cells_radial,
                              int zones_theta,
                              double diameter,
...
{
    if (zones_theta < 2 || cells_theta % zones_theta != 0 || cells_theta / zones_theta < 1
        || cells_radial < 1 || !std::isfinite(diameter) || diameter <= 0.0
        || !std::isfinite(outer_radius) || outer_radius <= 0.5 * diameter
        || !std::isfinite(radial_cluster_strength) || radial_cluster_strength < 0.0) {
        throw std::invalid_argument(
    ...
    const auto cylinder_zone_name
        = [](int zone_index) { return "CylinderZone" + std::to_string(zone_index + 1); };

    int file = 0;
    check_cgns(cg_open(path.c_str(), CG_MODE_WRITE, &file), "cg_open cylinder O-grid");
    try {
        int base = 0;
        check_cgns(cg_base_write(file, "WCNSCylinderOGrid", 2, 2, &base),
        ...
        for (int zone_index = 0; zone_index < zones_theta; ++zone_index) {
            cgsize_t size[6] = {
                ...
                static_cast<cgsize_t>(cells_radial),
                0,
                0,
            };
            int zone = 0;
            ...
            std::vector<double> y(count);
            for (int j = 0; j < nj; ++j) {
                const double eta = static_cast<double>(j) / static_cast<double>(cells_radial);
                const double radial_fraction
                    = cylinder_radial_coordinate(eta, radial_cluster_strength);
                const double radius
                    = cylinder_radius + (outer_radius - cylinder_radius) * radial_fraction;
                for (int i = 0; i < ni; ++i) {
                    ...
                    const double theta = -2.0 * pi * static_cast<double>(global_i)
            ...
            write_boundary(file, base, zone, "cylinder", {1, 1, ni, 1}, BCWall);
            write_boundary(file, base, zone, "farfield", {1, nj, ni, nj}, BCFarfield);
        }
...
int main(int argc, char** argv)
{
    if (argc != 13 && argc != 12 && argc != 9 && argc != 8 && argc != 6 && argc != 5) {
        std::cerr << "usage: wcns_generate_release_cgns <output.cgns> <dimension> "
    ...
    try {
        ...
        if (argc == 5 && std::string(argv[1]) == "invalid-one-sided") {
            ...
            output = argv[2];
            generate_periodic_square(output,
                                     parse_positive(argv[3], "cells_i"),
                                     parse_positive(argv[4], "cells_j"),
                                     parse_positive_real(argv[5], "length"));
        } else if (argc == 8 && std::string(argv[1]) == "warped-periodic-square") {
            output = argv[2];
            generate_periodic_square(output,
                                     parse_positive(argv[3], "cells_i"),
                                     parse_positive(argv[4], "cells_j"),
        ...
        } else if ((argc == 12 || argc == 13) && std::string(argv[1]) == "periodic-channel") {
            output = argv[2];
            generate_periodic_channel(output,
                                      parse_positive(argv[3], "cells_i"),
                                      parse_positive(argv[4], "cells_j"),
            ...
            output = argv[2];
            generate_cylinder_o_grid(output,
            ...
            generate(output,
                     ...
                     parse_positive_real(argv[7], "length_y"),
                     1.0);
        } else if (argc == 9) {
            output = argv[1];
            generate(output,
                     parse_positive(argv[2], "dimension"),
                     parse_positive(argv[3], "cells_i"),
                     parse_positive(argv[4], "cells_j"),
                     parse_positive(argv[5], "cells_k"),
                     parse_positive(argv[6], "zones_i"),
