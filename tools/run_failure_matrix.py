...
def execute(
    ...
    if result.returncode not in expected:
        raise RuntimeError(f"expected exit {expected}, got {result.returncode}: {log}")
...
def assert_no_final_solution(directory: Path) -> None:
    ...
    forbidden = [
        ...
        if ".field." in path.name or ".checkpoint." in path.name or ".manifest." in path.name
    ]
    if forbidden:
...
def main() -> int:
    ...
    failures.extend(
        [
            (
                "unknown-key",
                write_config("unknown-key", base + "unknown.option = 1\n"),
                parser_output,
                "unknown configuration key",
            ),
            (
                "duplicate-key",
                write_config("duplicate-key", base + "run.cfl = 0.2\n"),
                parser_output,
                "duplicate configuration key",
            ),
            (
                "direct-re",
                write_config("direct-re", base + "Re = 1000\n"),
                parser_output,
                "derived values and cannot be configured",
            ),
            (
                "direct-ma",
                write_config("direct-ma", base + "Ma = 0.2\n"),
                parser_output,
                "derived values and cannot be configured",
            ),
        ]
    ...
    for name, config, output, fragment in failures:
        for rank in ranks:
            ...
            records.append(execute(command(config, rank), log, (1,), args.timeout, fragment))
    ...
    records.append(
        execute(
            command(conflict_config, 1),
            root / "output-conflict-first.log",
            (0,),
            args.timeout,
    ...
    for rank in ranks:
        records.append(
            execute(
                command(conflict_config, rank),
                root / f"output-conflict-r{rank}.log",
                (1,),
                args.timeout,
    ...
    checkpoint_config = write_config(
        "checkpoint-source",
        config_text(
            "checkpoint-source",
            valid_mesh,
    ...
    records.append(
        execute(
            command(checkpoint_config, 1),
            root / "checkpoint-source.log",
            (2,),
            args.timeout,
            "maximum_steps",
        )
    ...
    for rank in ranks:
        ...
        records.append(
            execute(
                command(mismatch_config, rank),
                root / f"restart-mismatch-r{rank}.log",
                (1,),
                args.timeout,
                "signature differs",
            )
    ...
    summary = {
        ...
        + [
            ...
            "restart-signature",
            "numerical-failure",
            "unknown-dependency-unit",
            "cyclic-dependency-unit",
        ],
        "records": records,
    }
